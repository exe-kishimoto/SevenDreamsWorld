...
- **キャラ（マスコット7体）は World に1体だけ出す**。`mascots[]` に7体とも登録し、出ているのは
  `mascotIdx` の1体（`showMascot`）。`updateMascots` が `MASCOT_SEC` 秒ごとに入れ替えを試みるが、
  **`outOfView` が真＝視界の外で、かつ会話中でないときしか入れ替えない**（目の前で消さないため）。
  出ていないキャラは `talkMeshes()` が `visible` で弾くので狙えない。**人（住人）はこの制限の対象外**。
- **空の板ポリ（飛行機・鳥）の向きに注意**。来場者は南(-z)から北を向くので、**画面の右＝ワールドの -x**。
