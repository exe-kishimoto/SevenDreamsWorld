...
- **画面は素材の縦横比に自動で合わせる**（`fitScreen`）。枠と比率が違う画像でも
  伸びずに枠内へ収まり、上下（または左右）に黒みが残るだけ。
- 配置：**横型2面（動画・静止画）は参道の左右**（±13, -13、南の入口を向く）、
  **縦長1面は奥＝銅像の北側**（0, 19、広場の中心＝南を向く）。
  位置・向き・大きさは `addMonitor(...)` 呼び出しで調整。
...
- **操作モードは「端末がタッチ対応か」で決めないこと**。`navigator.maxTouchPoints > 0` で判定すると、
  **タッチパネル付きの PC が問答無用でスマホ UI になり、マウスの照準クリックが効かなくなる**。
  端末の能力（`hasTouch`）は**切替ボタンを出すかどうか**の判断にだけ使う。
  逆に**タッチできない端末をタッチ操作にしてはいけない**（スティックを触れず操作不能になる）。
- モードの決まり方は3段階：①起動時は `(pointer: coarse)` で仮決め →
  ②**スタート画面をタップ／クリックした `pointerType` で確定** →
  ③プレイ中は `setInputMode` でいつでも切替（右上のボタン／`T` キー）。
  スマホ＋キーボードのような組み合わせにも対応できる。
- 切替まわりで踏みやすい穴が3つある：
  1. **マウス操作中はポインターロック中で右上のボタンを押せない**（クリックは3D側の
     「話しかける」に吸われる）。だから `T` キーの導線が要る。
  2. ロック解除時にスタート画面へ戻す処理があるので、**切替でのロック解除では戻さない**
     （`switchingMode` フラグで抑制）。
  3. **スマホはポインターロック非対応**。マウス操作にすると操作不能になるので、
     `pointerlockerror` を受けてタッチ操作へ自動で戻す。
- タッチ操作中でもキーボードは効く（`keys` は常に拾い、`active()` は `mobileActive` で真になる）。
