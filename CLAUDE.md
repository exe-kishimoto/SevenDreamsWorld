...
- **空** = `addSky` のグラデーションドーム。**上へいくほど濃い水色、地平線でほぼ白い紙**。
  桜色（ピンク）は入れない。空と雲は `toneMapped: false`（下記の理由）。
...
- **`collides(x, z, pad)` の `pad` に体の太さを渡すこと**。点で判定すると壁の面に達するまで進めてしまい、
  顔や立ち絵が物体にめり込む。プレイヤーは `PLAYER_R`、住人は 0.7 で判定している。
...
- **色はブランドレッド `#E60013` だけ。ピンクを出さないこと**。ピンクは足した覚えがなくても出る：
  1. **トーンマッピング**。ACES は明るい面の彩度を落とすので、照らされた赤がサーモンピンクに転ぶ。
     `renderer.toneMapping = THREE.NoToneMapping` にしてクランプさせる（＝赤は赤のまま）。
  2. **半透明の赤**。`rgba(230,0,19,0.2)` のような薄い赤を白地に敷くと、それはピンクそのもの。
     色は `SD_RED_CSS` のまま、**線を細く**して面の重さを調整する。
  3. **光の色**。ライトに桜色（`0xffe3e7` など）を混ぜると白い紙がピンクに染まる。光は白にする。
  4. 白飛びも彩度を落とすので、ライトの合計強度は 1 を大きく超えさせない。
