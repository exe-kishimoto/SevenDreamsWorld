...
- **立ち絵の向きを固定しないこと**。厚みゼロの板なので向きを固定すると**真横に回り込んだ瞬間に消える**。
  `faceCamera(rec, vx, vz)` が毎フレーム Y軸だけこちらへ向ける（吹き出しと同じ考え方）。
  左右反転は**進行方向ベクトル `(vx,vz)` と「画面右」ベクトルの内積の符号**で決める。
  絵は左向きなので、画面右へ進んで見えるときだけ `setFlip(-1)` で鏡にする。
  **x 成分だけで判定してはいけない**（斜め・奥行き方向に歩くとき、カメラが東西を向いたときに
  判定が逆になり「後ろ歩き」に見える）。手前／奥へ歩くときは左右が決まらないのでデッドゾーンを置き、
  前の向きを保ってちらつかせない。**文字入りの飛行機だけは固定向きのまま**。
